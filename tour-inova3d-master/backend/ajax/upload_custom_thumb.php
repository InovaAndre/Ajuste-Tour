...
if(isset($_FILES) && !empty($_FILES['file']['name'])){
    ...
    if(in_array($ext,$allowed_ext)){
        ...
        $moved = move_uploaded_file($_FILES['file']['tmp_name'],dirname(__FILE__).'/../../viewer/panoramas/thumb_custom/'.$name);
        if($moved) {
            try {
                $image = new ImageResize(dirname(__FILE__).'/../../viewer/panoramas/thumb_custom/'.$name);
                $image->quality_jpg = 100;
                $image->interlace = 1;
                $image->resizeToBestFit(213,120,true);
                $image->gamma(false);
                $image->save(dirname(__FILE__).'/../../viewer/panoramas/thumb_custom/'.$name);
