...
if(isset($_POST['paypal_client_id'])) {
    ...
    if($paypal_client_id!="keep_paypal_client_id") {
        $mysqli->query("UPDATE svt_settings SET paypal_client_id='$paypal_client_id';");
    }
    if($paypal_client_secret!="keep_paypal_client_secret") {
        $mysqli->query("UPDATE svt_settings SET paypal_client_secret='$paypal_client_secret';");
    }
}
...
if($result) {
    if($result->num_rows>0) {
        while($row = $result->fetch_array(MYSQLI_ASSOC)) {
            ...
            array_push($plans_array,array("id"=>$id,"id_plan_paypal"=>$id_plan_paypal,"name"=>$name,"price"=>$price,"currency"=>$currency,"frequency"=>$frequency,"interval_count"=>$interval_count));
        }
    }
}
...
foreach ($plans_array as $plan) {
    $id_plan = $plan['id'];
    if(($id_plan_p!=0) && ($id_plan!=$id_plan_p) && ($plan['frequency']!='recurring')) {
        continue;
    }
    ...
    if(empty($id_plan_paypal)) {
        //CREATE PLAN
        $id_plan_paypal = create_plan($access_token,$id_product_paypal,$name,$currency,$price,$interval_count);
...
$query = "UPDATE svt_settings SET paypal_enabled=1;";
$result = $mysqli->query($query);
if($result) {
...
function create_plan($access_token,$id_product_paypal,$name,$currency,$price,$interval_count) {
    global $url_paypal;
    $plan_id = null;
    $ch = curl_init();
